...
| `08_backup/lolipopbackup/` | 既存サイトの保全バックアップ | 比較、参照、移行漏れ確認に使う。新サイトから直接参照しない |
...
| `08_backup/` | 既存サイトの保全バックアップ |

リポジトリの運用方針が変わらない限り、ルート直下に新しい作業ファイルを増やさない。
`08_backup/` は既存サイト保全のための例外であり、現行実装の配置先ではない。
...
本文内の軽い上余白はインラインstyleではなく、以下のユーティリティクラスで扱う。

| クラス | 値 | 用途 |
| --- | ---: | --- |
| `.space-top-xs` | `12px` | 短いカード内リスト |
| `.space-top-sm` | `14px` | 実績カード内の短いリスト |
| `.space-top-md` | `18px` | 標準的な本文内リスト |
| `.space-top-lg` | `24px` | カードグリッドなどのまとまり |
